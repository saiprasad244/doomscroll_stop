...
For production environments, do not run Flask directly via debug mode. Instead, use a WSGI server like **Gunicorn** (Mac/Linux) or **Waitress** (Windows).

1.  Install Waitress:
    ```bash
    pip install waitress
    ```
2.  Create a `wsgi.py` in the root:
    ```python
...
3.  Run the production server using waitress:
    ...
    ```
4.  Deploy to cloud hosting providers like **Render**, **Heroku**, or **AWS Elastic Beanstalk** setting the environment variables `JWT_SECRET_KEY` and `OPENAI_API_KEY`.

### 2. Deploying the Frontend (React)

1.  Navigate to the `frontend/` folder and compile the production build:
    ```bash
    npm run build
    ```
2.  Vite generates static build assets in `frontend/dist/`.
3.  Deploy the static folder to static hosting services like **Vercel**, **Netlify**, **GitHub Pages**, or **AWS S3**.
4.  Ensure you change the backend URL in `frontend/src/` components (e.g., `http://localhost:5000` to your production Flask domain URL).
